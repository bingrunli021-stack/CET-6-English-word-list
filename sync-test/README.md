# CET-6 独立同步测试版

测试地址：https://bingrunli021-stack.github.io/CET-6-English-word-list/sync-test/
现有根目录网站及旧 Sites 地址保持原样。测试页、登录会话、学习缓存、同步队列和云端数据表均独立。

## 词义校订

5651 条原始记录合并为 3991 个不重复词头。`translations-reviewed.js` 为每个词头提供经过统一格式清理和质量检查的主释义；常见多义词、错词性、错译和过窄释义优先人工修正，原始重复释义仍保留在学习卡片中供展开核对。

自动审计参考 ECDICT 的考试标签、词频和中英文释义。ECDICT 采用 MIT License，Copyright (c) 2025 Linwei。当前版本没有获得新东方、星火英语或华研最新版词汇书的完整内容，因此不将校订结果标注为三家教辅原文或三方一致译文；收到合法提供的对应材料后可继续逐词对照。

## 模糊单词专项

底部“模糊”入口直接读取现有 `records[*].status === 'hard'` 的词，不新增或复制学习进度。它不受复习到期日限制，按累计错误次数优先展示，每轮最多显示 20 个并在处理后自动补位。“这次记住了”会移出模糊清单并延长复习间隔；“仍然模糊”会保留该词、增加一次错误并安排明天复习，同时本轮先进入下一个词。登录后这些变化沿用现有逐词云同步。

## 今日新词检测

“今日”页的“检测今日新词”会从当天已经固定的推荐清单中出题，不改变每日目标，也不会增加新的单词。每题显示英文并提供四个中文释义选项；首次答对的未标记新词会进入正常的 1 天复习间隔，答错词会标记为“模糊”并安排明天复习。完成整轮后可只补测本轮错词，也可重新检测今日全部单词。补测答对用于即时强化，但不会抹去本轮首次答错记录。

## 未熟练单词专项

底部“未熟”入口筛选所有已经接触、状态不是 `new` 且 `level < 4` 的单词，即现有统计中尚未“稳定掌握”的 Level 1—3 单词。练习按等级较低、累计错误较多的顺序进行，每轮最多 20 个，支持继续下一组和重练本轮未记住的词。该专项的回答、轮次与重练队列只保留在当前页面会话中，不调用进度保存，也不修改单词的 `level`、`status`、`due`、`wrong`、`learnedAt`、每日计划或总进度。

## 数据结构
`public.cet6_sync_test_records`：联合主键 `(user_id,key)`；字段 `value jsonb`、`updated_at bigint`（编辑时毫秒）、`mutation_id text`（相同时刻稳定排序）、`deleted boolean`（删除标记）、`received_at timestamptz`。
`key` 是 JSON 路径：每个单词一行，history/todayPlans 每日期一行，听力完成项每套一行；其余设置分别一行。单词状态、等级、复习日期等作为整体提交。笔记为一条记录。

## 同步
修改立即进入 localStorage 持久队列；打开、学习、联网、获得焦点、恢复可见触发同步；可见页面每15秒补查。RPC 在数据库事务中逐行条件 UPSERT，只接受更晚的编辑时间，相同时按 mutation_id 排序。旧请求晚到不能覆盖新记录。每个请求仅确认自己携带的操作，不删除请求期间新增的操作。空设备导入的历史种子只补缺失行。不会提交整个学习 JSON 覆盖云端。

退出只清除测试版会话；每个账户的本地缓存及待同步队列保留，重新登录后恢复。首次在线加载后 Service Worker 缓存页面、同步程序及固定版本 Supabase SDK 2.57.4，支持离线重新打开。外部真题和发音服务仍取决于网络与设备语音支持。

## JSON 恢复
旧网站：设置 → 导出学习进度。在测试网站：设置 → 导入学习进度，选择 JSON，再登录同一账号。导入是一次新的编辑，备份中同名记录以导入内容恢复，备份未含的现有单词保留。登录后自动同步。导出保留词汇、设置、听力和同步元数据。不导出登录凭据。
测试网址不能直接读取旧 Sites 域名的浏览器数据，需要通过 JSON 转移。

## 验证
`node --test tests/*.test.cjs`：同步引擎 8 项回归，以及 3991 个校订词头的完整性、格式、代表性错译修正与离线缓存检查。
真实 Supabase 事务测试：RPC条件合并、新旧冲突、不同词保留、空种子保护、跨账户读取隔离通过；全部测试写入回滚。
真实 Supabase + 三个独立学习存储的浏览器客户端验收已通过：A→B、不同词保留、同词较晚修改、模拟断网自动补传、JSON词汇及听力笔记导入、真实退出并安全重新登录恢复。详见 ACCEPTANCE.md。这不代表已操作用户的三台实体设备。
客户端采用单调编辑时间；长期离线设备时钟严重不准时，无法保证按现实世界时间排序。

测试账号目前包含验收产生的6个词条和一条测试笔记；正式使用前请从旧网站导出并导入真实 JSON 备份。旧网站和原云端数据保持不变。
